#ifndef GLUE_GLBUFFER_HPP_INCLUDED
#define GLUE_GLBUFFER_HPP_INCLUDED
...
{
public:
	explicit VertexBuffer(const std::vector<vec4> &v, GLenum usage = GL_STATIC_DRAW);
	explicit VertexBuffer(const std::vector<vec3> &v, GLenum usage = GL_STATIC_DRAW);
	explicit VertexBuffer(const std::vector<vec2> &v, GLenum usage = GL_STATIC_DRAW);
