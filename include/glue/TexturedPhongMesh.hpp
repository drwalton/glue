#ifndef GLUE_TEXTUREDPHONGMESH_HPP_INCLUDED
#define GLUE_TEXTUREDPHONGMESH_HPP_INCLUDED
...
public:
	...
	void specularColor(const glue::vec3 &color);

	//!\brief Position of light (in world space).
	void lightWorldPos(const glue::vec3 &p);
	const glue::vec3 &lightWorldPos() const;

	virtual void render();

	void render(glue::ShaderProgram *program);
...
private:
	...
	bool drawShadow_;
	vec4 shadowColor_;
	glue::vec3 specularColor_, lightWorldPos_;
};
