	...
	shadowColor_(0.6f, 0.6f, 0.6f, 0.4f)
{
	if (nInstances == 0) {
		shader.reset(new ShaderProgram({
			GLUE_SHADER_DIR + "TexturedPhongMesh.vert",
			GLUE_SHADER_DIR + "TexturedPhongMesh.frag"
		}));
...
void TexturedPhongMesh::specularColor(const glue::vec3 & color)
{
	specularColor_ = color;
}

void TexturedPhongMesh::lightWorldPos(const glue::vec3 & p)
{
	lightWorldPos_ = p;
}

const glue::vec3 &TexturedPhongMesh::lightWorldPos() const
{
...
void TexturedPhongMesh::render(glue::ShaderProgram *program)
{
	...
	if(s) {
		if(!shadowShader) {
			shadowShader.reset(new ShaderProgram({
				GLUE_SHADER_DIR + "TexturedPhongMeshShadow.vert",
				GLUE_SHADER_DIR + "TexturedPhongMeshShadow.frag"
			}));
		}
	}
	drawShadow_ = s;
