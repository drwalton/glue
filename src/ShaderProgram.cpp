...
void checkForGLError(const std::string &exception)
{
	...
	if (err != GL_NO_ERROR) {
		//throw ShaderException(exception + " " + errname);
	}
}
...
{
	glProgramUniform2fv(program_, uniformLoc(name), 1, value.data());
}

void ShaderProgram::setUniform(const std::string &name, const dvec2 &value)
{
	glProgramUniform2dv(program_, uniformLoc(name), 1, value.data());
}

void ShaderProgram::setUniform(const std::string &name, const vec3 &value)
{
	glProgramUniform3fv(program_, uniformLoc(name), 1, value.data());
}

void ShaderProgram::setUniform(const std::string &name, const vec4 &value)
{
	glProgramUniform4fv(program_, uniformLoc(name), 1, value.data());
}

void ShaderProgram::setUniform(const std::string &name, GLfloat value)
{
	glProgramUniform1f(program_, uniformLoc(name), value);
}

void ShaderProgram::setUniform(const std::string &name, GLdouble value)
{
	glProgramUniform1d(program_, uniformLoc(name), value);
}

void ShaderProgram::setUniform(const std::string &name, const mat3 &value)
{
	glProgramUniformMatrix3fv(program_,
		uniformLoc(name), 1, GL_FALSE, value.data());
}

void ShaderProgram::setUniform(const std::string &name, const mat4 &value)
{
	glProgramUniformMatrix4fv(program_,
		uniformLoc(name), 1, GL_FALSE, value.data());
}

void ShaderProgram::setUniform(const std::string &name, GLint value)
{
	glProgramUniform1i(program_, uniformLoc(name), value);
}

void ShaderProgram::setUniform(const std::string &name, GLuint value)
{
	glProgramUniform1ui(program_, uniformLoc(name), value);
}

void ShaderProgram::setUniform(const std::string &name, const std::vector<float>& v)
{
	glProgramUniform1fv(program_, uniformLoc(name), GLsizei(v.size()), &(v[0]));
}

void ShaderProgram::setUniform(const std::string &name, const std::vector<vec3>& v)
{
	glProgramUniform3fv(program_, uniformLoc(name), GLsizei(v.size()), &(v[0][0]));
}
...
void ShaderProgram::validate()
{
	...
	if(val != GL_TRUE) {
		...
		delete[] errorLog;
		throw ShaderException("Program validation failed!: \"" +
			errorString + "\"");
	}
}
...
GLuint ShaderProgram::compileShader(const std::string &filename, GLenum type)
{
	...
	if (status == GL_FALSE) //Compilation failure.
	{
		...
		throw ShaderException("The shader source obtained from\n"
						  "    file: " + filename + "\n"
...
mat3 ShaderProgram::getNormMatrix(const mat4 &transform)
{
	mat3 rotation = transform.block<3, 3>(0, 0);
	return rotation.inverse().transpose();
}
